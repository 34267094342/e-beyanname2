...
* CLEAR mt_detail_alv.
    ...
    SELECT SINGLE fieldname
            FROM ztax_t_tevit
            INTO @lv_ita.
    "--- kırılım (breakdown) tespiti için sadece gerektiğinde ayrı bir
    "    internal tabloya (lt_bseg_ita) BSEG-T metin/atama satırları okunur.
    "    find_document / lt_bseg mevcut haliyle (BSEG okumadan) çalışmaya devam eder,
    "    böylece bu yeni mantık mevcut raporu ETKİLEMEZ.
    CLEAR lt_bseg_ita.
...
* INTO TABLE @DATA(lt_indirim).
    ...
    j~fiscalperiod         = @p_monat
          ...
          AND j~taxcode              <> ''
          AND
    j~transactiontypedetermination = 'ZTA'  " ← WHERE'a taşındı
...
*
              ...
              "    belge satırı (BSEG-T) metin/atama alanına göre eşleme yapılır.
              ...
              IF lv_map_count > 1.
                ...
                " ET_BSET belge bazında (bukrs, belnr, gjahr) çekildiği için doğrudan eşleştirilebilir:
                ...
                IF sy-subrc = 0.
                  ASSIGN COMPONENT lv_ita_eff OF STRUCTURE ls_bseg TO <fs_ita_value>.
...
* AND ktosl IN lr_ktosl.
                ...
                "           HWSTE yerine HWBAS alanında
                "" ""     geldiğinden matrah şişiyor,
                ...
                IF ls_bset-hkont(3) = '391' AND ls_bset-hwbas <> 0.
                  ...
                  " pozitif kalmalı → abs() ile düşülecek
                  CLEAR ls_bset-hwbas.
                ENDIF.
        ...
        WHEN '013'.
          ...
          IF ls_map-kiril1 = '99'.
            LOOP AT lt_bset INTO ls_bset WHERE   blart = ls_map-blart.
              ...
              lv_oran_int = abs( ls_bset-kbetr ) .
              ls_collect-oran = lv_oran_int.
              SHIFT ls_collect-oran LEFT DELETING
LEADING space.
              ls_collect-matrah = ls_bset-hwbas .
              ls_collect-vergi  = ls_bset-hwste .
...
* CLEAR lv_kiril2.     " ESKİ - SPACE yapıyor, eşleşmiyor
        lv_kiril2 = '000'.
        " YENİ - mt_collect'teki başlık satırıyla eşleşir
