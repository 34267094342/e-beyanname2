...
* DATA lt_rbkp TYPE SORTED TABLE OF mty_rbkp WITH UNIQUE KEY belnr gjahr.
    ...
    "--- lt_bkpf/et_bkpf doldurma: mevcut et_bset/mt_collect akışını etkilemez,
    "    sadece kırılım (lv_ita/lt_bseg_ita) mantığının çalışabilmesi için eklendi.
    SELECT i_journalentry~CompanyCode AS bukrs,
          ...
          i_journalentry~ReversalReferenceDocumentCntxt AS aworg_rev,
               ...
               WHERE i_journalentry~CompanyCode EQ @p_bukrs
                 AND i_journalentry~FiscalYear
    EQ @p_gjahr
                 AND i_journalentry~FiscalPeriod IN @mr_monat
                 AND i_journalentry~IsReversed = @space
                 AND i_journalentry~IsReversal = @space
                INTO TABLE @et_bkpf.
    "--- /lt_bkpf doldurma --- (eski RETURN mantığı bilinçli olarak eklenmedi,
    "    et_bset/mt_collect akışını bozmamak için sy-subrc kontrolü yapılmıyor.)
...
* ENDIF.
    ...
    IF is_read_tab-bset EQ abap_true.
      " ET_BSET doldururken belge numaraları (bukrs, belnr, gjahr) dahil edildi,
      " böylece lv_ita mantığında BSEG eşleştirmesi doğru çalışabilir.
      SELECT
